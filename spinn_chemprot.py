...
def _evaluate_on_dataset(mean_loss, accuracy, chemprot_data, batch_size, model, use_gpu):
  ...
  f1score = metrics.f1_score(test_labels_whole, tf.cast(tf.argmax(test_logits_whole, 1), tf.int32), average='micro')
#  return mean_loss.result().numpy(), accuracy.result().numpy(), f1score, test_labels_whole, test_logits_whole, _pmids, _ent1s, _ent2s
  return mean_loss, accuracy, f1score, test_labels_whole, test_logits_whole, _pmids, _ent1s, _ent2s
...
def _get_dataset_iterator_noShuffle(chemprot_data, batch_size):
  ...
  with tf.device("/device:CPU:0"):
    # Some tf.data ops, such as ShuffleDataset, are available only on CPU.
    dataset = tf.data.Dataset.from_generator(
        chemprot_data.get_generator(batch_size, shuffle=False),
        (tf.int64, tf.int64, tf.int64, tf.int64, tf.int64, tf.int64))
...
def train_spinn(embed, train_data, dev_data, config):
  ...
  with tf.device(device), \
    ...
    for epoch in xrange(config.epochs):
      bbs_strs = ["Epoch : %s"%epoch, log_header]
      batch_idx = 0
      for label, word_ids, trans, pmids, ent1s, ent2s in _get_dataset_iterator(
        ...
        if iterations % config.save_every == 0:
          ...
          #tfe.Saver(all_variables).save(os.path.join(config.logdir, "ckpt"), global_step=global_step)
        ...
        if iterations % config.dev_every == 0:
          dev_loss, dev_frac_correct, dev_f1, dev_labels, dev_logits, dev_pmids, dev_ent1s, dev_ent2s = _evaluate_on_dataset(
              dev_mean_loss, dev_accuracy, dev_data, config.batch_size, model, use_gpu)

          dev_loss = dev_loss.result().numpy()
          dev_frac_correct = dev_frac_correct.result().numpy()
          dev_log_str = dev_log_template.format(
              time.time() - start,
          ...
          print(dev_log_str)
          print(str(dev_f1))#(not the micro-averaged f1 score, just rough reference)
          bbs_strs.append(dev_log_str)
          bbs_strs.append(str(dev_f1))
          ...
          log_str = log_template.format(
              time.time() - start,
              epoch, iterations, 1 + batch_idx, train_len,
              100.0 * (1 + batch_idx) / train_len,
              mean_loss_val, " " * 8, accuracy_val * 100.0, " " * 12)
          print(log_str)
          bbs_strs.append(log_str)
...
def test_spinn(embed, test_data, config, entire=True):
  ...
  with tf.device(device), \
    ...
    if len(config.ckptnum) != 0:
        ckpt = config.logdir+"/ckpt-%s"%str(config.ckptnum)
    print(ckpt)
    tfe.restore_network_checkpoint(model, ckpt)

    start = time.time()
    iterations = 0
    test_mean_loss = tfe.metrics.Mean()
    test_accuracy = tfe.metrics.Accuracy()

    test_loss, test_frac_correct, test_f1, test_labels, test_logits, test_pmids, test_ent1s, test_ent2s = _evaluate_on_dataset(
        test_mean_loss, test_accuracy, test_data, config.batch_size, model, use_gpu)

    print(log_header)
    print(test_log_template.format(
        time.time() - start,
        0, 0, 1, 0,
        1,
        0, test_loss.result().numpy(),
...
def main_genefusion(_):
  ...
  setproctitle.setproctitle("sunkyu_SPINN_%s"%FLAGS.genefusion_expstr)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--d_out", type=int, default=2, # genefusion : 2
  ...
  parser.add_argument("--force_cpu", action="store_true", dest="force_cpu",
                      ...
                      "available.")
  parser.add_argument("--genefusion_expstr", type=str,
                      help="get exp string in exp_paths.ini")
  parser.add_argument("--ckptnum", type=str, default="",
